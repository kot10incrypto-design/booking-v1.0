...
export const AtelierProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [currentCustomer, setCurrentCustomer] = useState<ClientProfile>(() => {
    try {
      const saved = localStorage.getItem('atelier_current_customer_v1');
      if (saved) {
        return JSON.parse(saved);
      }
    } catch (e) {
      console.error('Error loading current customer from localStorage', e);
    }
  ...
  const [accoutrements, setAccoutrements] = useState<Accoutrement[]>(ACCOUTREMENTS_DATA);
  ...
  // Save currentCustomer changes to localStorage
  useEffect(() => {
    try {
      localStorage.setItem('atelier_current_customer_v1', JSON.stringify(currentCustomer));
    } catch (e) {
  ...
  const loginManagement = (user: string, pass: string): { success: boolean; error?: string } => {
    ...
    const validUsers = ['admin', 'manager', 'مدیر', 'رویال', 'royal', 'barber'];
    const validPasswords = ['admin', '1234', 'admin123', 'royal1234', '123456', 'royal'];
    ...
    return {
      success: false,
      error: 'نام کاربری یا رمز عبور نادرست است. (نام کاربری: admin / رمز: 1234)',
    };
  };
  ...
  const createOnlineBooking = (data: {
    ...
    // Ensure current customer session is set to the booker
    if (customer) {
