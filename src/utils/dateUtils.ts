...
/**
 * Returns full Persian date string for a day number dynamically based on current real-time month and year.
 */
export function getPersianDateForDay(dayNumber?: number, monthName?: string, year?: number): string {
  ...
  const targetDay = dayNumber !== undefined ? dayNumber : current.dayNumber;
  const targetMonth = monthName || current.monthName;
  const targetYear = year || current.year;

  if (targetDay === current.dayNumber && targetMonth === current.monthName && targetYear === current.year) {
    return current.dateString;
  }

  return `${current.dayName}، ${toPersianDigits(targetDay)} ${targetMonth} ${toPersianDigits(targetYear)}`;
}
