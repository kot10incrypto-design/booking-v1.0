...
export const generateUpcomingDays = (count = 14): DayOption[] => {
...
// ── Master Salon Schedule Slots (From 09:00 AM to 22:00 PM) ───────
export const MASTER_TIME_SLOTS: string[] = [
  '09:00', '09:20', '09:30', '09:40',
...
export const getDayOccupiedRanges = (
  ...
  // Pre-booked appointments for realistic salon schedule
  const seedRanges: TimeRange[] = [
    { start: timeToMinutes('09:00'), end: timeToMinutes('09:40') },
    { start: timeToMinutes('14:00'), end: timeToMinutes('14:40') },
    { start: timeToMinutes('18:00'), end: timeToMinutes('18:40') },
  ];
  ranges.push(...seedRanges);

  return ranges;
};

/**
 * Calculates open gaps between existing bookings during salon operating hours (09:00 to 22:00).
 */
export const calculateScheduleGaps = (
  ...
  const salonOpen = 9 * 60; // 540 min (09:00)
  const salonClose = 22 * 60; // 1320 min (22:00)
...
export const checkSlotAvailability = (
  ...
  const salonClose = 22 * 60;

  if (slotEnd > salonClose) {
    return {
      time,
      isAvailable: false,
      conflictReason: 'مدت خدمت از ساعت پایان کار سالن (۲۲:۰۰) فراتر می‌رود',
