...
import { addMinutes, calculateBookingTotals, generateUpcomingDays } from '../utils/bookingUtils';
import { formatPrice } from '../utils/formatUtils';
import { toPersianDigits } from '../utils/dateUtils';
import { hapticLight, hapticSelection, hapticSuccess, hapticWarning, triggerHaptic } from '../utils/hapticUtils';
import { ArrowRight, Check, Coffee, Phone, ShieldCheck, User, VolumeX, Sparkles, CreditCard, Store, Lock } from 'lucide-react';
...
export const BookCheckoutView: React.FC<BookCheckoutViewProps> = ({
  ...
  const { currentCustomer, beverageOptions, activeBarber, activeChair, createOnlineBooking, settings } = useAtelier();
  const defaultBarber = activeBarber?.name || settings?.profile?.masterName || 'آرایشگر اختصاصی';
  ...
  const handleSubmit = () => {
    ...
    try {
      ...
      if (res.success && res.reservation) {
        ...
        // Fallback construct reservation object if reservation was not returned
        const newReservation: Reservation = {
          id: `res-${Date.now()}`,
          appointmentNumber: `#ROYAL-${Math.floor(1000 + Math.random() * 9000)}`,
          customerId: currentCustomer?.id || 'guest',
          customerName: customerName.trim(),
          customerPhone: customerPhone.trim(),
          serviceId: service.id,
          service,
          barberId: activeBarber?.id || 'barber-01',
          barberName: activeBarber?.name || defaultBarber,
          chairId: activeChair?.id || 'chair-01',
          chairName: activeChair?.name || 'سوئیت اصلی رویال',
          date: `${activeDay.weekday} ${activeDay.label}`,
          dayNumber: selectedDay,
          startTime: selectedTime,
          endTime,
          durationMinutes: totals.totalDuration,
          servicePrice: totals.servicePrice,
          accoutrementsPrice: totals.extrasPrice,
          tipPercentage: 0,
          tipAmount: 0,
          totalAmount: totals.total,
          depositAmount: Math.round(totals.total * 0.2),
          additionalAccoutrements: totals.selectedExtras,
          ...
          isQuietSession,
          selectedDate: `${activeDay.weekday} ${activeDay.label}`,
          selectedTime,
        };
  ...
  return (
    <AtelierShell id="book-checkout-container">
      ...
      <PriceSummary
        ...
        ctaLabel={isSubmitting ? 'در حال صدور کارت ورود...' : 'تأیید نهایی و صدور نوبت'}
        onCta={handleSubmit}
