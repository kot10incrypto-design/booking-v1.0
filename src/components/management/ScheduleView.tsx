    ...
    <div className="space-y-3.5 pb-16" dir="rtl">
          ...
          {weekDays.map((day, dayIndex) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className="font-mono font-bold my-0.5"
                  style={{ fontSize: '16px', fontStyle: 'normal' }}
                >
                  {day.dayNumber}
                </span>

                {/* Appointment Count Tag / Dot */}
                {count > 0 ? (
                  <span className={`text-[7px] font-mono px-1 py-0.2 rounded-full mt-0.5 ${
                    isSelected ? 'bg-stone-800 text-[#fbdcd9]' : 'bg-stone-200 text-stone-600'
                  }`}>
                    {toPersianDigits(count)}
                  </span>
                ) : (
                  <span className="w-1 h-1 rounded-full bg-stone-300 mt-1" />
                )}

                {/* Today Indicator */}
                {day.isToday && dayIndex !== 4 && (
                  <span className={`absolute top-1 left-1 w-1.5 h-1.5 rounded-full ${
                    isSelected ? 'bg-[#d88d85]' : 'bg-rose-500'
                  }`} />
                )}
              </button>
            );
      ...
      <div className="flex items-center justify-between gap-1.5">
        ...
        <span className="text-[9px] text-stone-500 font-mono shrink-0">
          {toPersianDigits(availableSlots.length)} بازه خالی
        </span>
      </div>
