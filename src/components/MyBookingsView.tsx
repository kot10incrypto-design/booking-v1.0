...
export const MyBookingsView: React.FC<MyBookingsViewProps> = ({
  ...
  // Helper to test customer matching
  const cleanPhone = (p?: string) => (p ? p.replace(/\D/g, '') : '');
  const activePhone = cleanPhone(currentCustomer.phone);

  const isCurrentCustomerAppointment = (apt: Appointment) => {
    if (apt.customerId && apt.customerId === currentCustomer.id) return true;
    if (apt.customerPhone && activePhone && cleanPhone(apt.customerPhone) === activePhone) return true;
    if (apt.customerName && apt.customerName.trim().toLowerCase() === currentCustomer.name.trim().toLowerCase()) return true;
    return false;
  };

  // Separate Active / Upcoming vs History
  const { activeBookings, historyBookings } = useMemo(() => {
    const userAppointments = appointments.filter(isCurrentCustomerAppointment);
