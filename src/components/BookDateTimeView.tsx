...
export const BookDateTimeView: React.FC<BookDateTimeViewProps> = ({
  ...
  // Auto-highlight and auto-select closest available slot if preferred time was booked
  useEffect(() => {
    if (!preferredTimeStatus.isAvailable && nearestResult?.nearest) {
      if (selectedTime === preferredTime || !selectedTime) {
        onSelectTime(nearestResult.nearest);
      }
    }
  }, [preferredTimeStatus.isAvailable, nearestResult?.nearest, selectedDay]);
  ...
  return (
    <AtelierShell id="book-datetime-container">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="mt-1 text-[11px] leading-5 text-amber-900">
                  با محاسبه فواصل خالی میان نوبت‌های موجود و مدت زمان کل خدمات شما (
                  {toPersianDigits(totals.totalDuration)} دقیقه)،{' '}
                  <strong className="font-black text-stone-900">
                    نزدیک‌ترین نوبت آزاد (ساعت {toPersianDigits(highlightedNearestSlot || '')})
                  </strong>{' '}
                  به صورت خودکار برای شما انتخاب و هایلایت شد.
                </p>
            ...
            <div>
              ...
              <div className="grid grid-cols-4 gap-2">
                {morningSlots.map(({ slot, isAvailable }) => {
                  ...
                  const isAttemptedTaken = takenSlotAttempted === slot;
            ...
            <div>
              ...
              <div className="grid grid-cols-4 gap-2">
                {eveningSlots.map(({ slot, isAvailable }) => {
                  const isSelected = selectedTime === slot;
                  const isNearest = highlightedNearestSlot === slot && isAvailable;
                  const isAttemptedTaken = targetBookedTime === slot && !isAvailable;
      ...
      <PriceSummary
        ...
        ctaDisabled={!selectedTime || !currentSlotStatus?.isAvailable}
        disabledHint={
          !selectedTime
